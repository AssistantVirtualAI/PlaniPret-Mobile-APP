...
/**
 * Send using a pre-generated OAuth2 access token (e.g. from `gcloud auth print-access-token`).
 * Use when service account key creation is blocked by org policy.
 * Token must be refreshed before expiry (1 hour) — see scripts/refresh-fcm-token.sh.
 */
export async function sendFcmDataMessageWithToken(
  accessToken: string,
  projectId: string,
  token: string,
  ...
  try {
    ...
    const unregistered = res.status === 404 || /UNREGISTERED|NOT_FOUND|INVALID_ARGUMENT/i.test(text);
    // 401 = token expired — caller should refresh FCM_ACCESS_TOKEN in Supabase secrets
    if (res.status === 401) return { ok: false, status: 401, error: "FCM_ACCESS_TOKEN_EXPIRED — run: gcloud auth print-access-token and update Supabase secret" };
